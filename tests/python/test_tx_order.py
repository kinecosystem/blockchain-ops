...
def get_latest_ledger(client):
    ...
    return client.horizon.ledgers(params=params)['_embedded']['records'][0]['sequence']
...
async def main():
    ...
    initial_ledger = get_latest_ledger(client)
    print(f'Initial ledger: {initial_ledger}')
    while initial_ledger == get_latest_ledger(client):
        time.sleep(0.5)
    ...
    # # Secpnd ledger should have 1 whitelisted tx, and 1 non-whitelisted
